...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    $qry = mysqli_query($conn, "SELECT atm_pin FROM atm_table WHERE atm_id='$aid'");

    if ($qry) {
        $Data = mysqli_fetch_assoc($qry);
        $correctPin = $Data['atm_pin'];

        if ($enteredPin == $correctPin) {
            header("Location: admin-balance_enquiry.php");
            exit();
        } else {
            ...
            echo '<script>
                document.addEventListener("DOMContentLoaded", function() {
                    Swal.fire({
                        ...
                        window.location.href = "admin-balance-pin.php";
                    });
                });
            </script>';
            exit();
